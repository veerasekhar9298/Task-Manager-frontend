...
import {DragDropContext, Droppable,Draggable } from "react-beautiful-dnd"
  ...
  function CheckBox (props){
      ...
      const handleDragEnd = (result) => {
        ...
        //   axios
        //     .put(`http://127.0.0.1:3850/api/tasks/${draggedTask._id}`, draggedTask)
        //     .then((response) => {
            ...
            Object.keys(datesState).length >0 &&  Object.keys(datesState).map((ele,i)=>{
                            return (<div key ={i}>
                                  ...
                                  <Droppable droppableId={`${ele}`}>
                                    {(provided)=>(
                                      <div  className='flex-center bg-danger bg-opacity-50 rounded-5 shadow' ref={provided.innerRef} {...provided.droppableProps}>
                                        {
                                          datesState[ele].map((ele,i2)=>{
                                                return (
                                                    <Draggable key = {i2} draggableId={`${ele._id}`} index={i2}>
                                                      {(provided)=>(
                                                         <div className="tasks" ref={provided.innerRef} {...provided.dragHandleProps}{...provided.draggableProps}>
                                                           <TaskItem key={i2} {...ele} />
                                                         </div>
                                                      )}
                                                    </Draggable>
